...
export default function UploadModal({ isOpen, onClose, onUpload, userLocation }) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-md mx-4 rounded-3xl p-0 overflow-hidden  bg-gray-50">
        <DialogHeader className="px-6 py-4 bg-gray-50">
        ...
        <form onSubmit={handleSubmit} className="px-6 py-4 space-y-4">
          <div className="space-y-2">
            <Label htmlFor="title">歌曲名稱</Label>
            <Input id="title" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="輸入歌曲名稱" required className="rounded-xl" />
          </div>
          <div className="space-y-2">
            <Label htmlFor="artist">演唱者</Label>
            <Input id="artist" value={artist} onChange={(e) => setArtist(e.target.value)} placeholder="輸入演唱者" required className="rounded-xl" />
          </div>
          
          {/* 這邊還沒辦法連接 */}
          <div className="space-y-2">
            <Label htmlFor="youtube">YouTube 連結</Label>
            <Input id="youtube" type="url" value={youtubeUrl} onChange={(e) => setYoutubeUrl(e.target.value)} placeholder="https://www.youtube.com/watch?v=..." required className="rounded-xl" />
          </div>
